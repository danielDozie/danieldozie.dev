...
type DATA = {
  data: HomeBlogSectionProps
}

export const HomeBlogSection = ({data}:DATA) => {
  const { block, custom_blogs }: HomeBlogSectionProps = data;
  return (
    <>
      <div className="h-full mx-auto max-w-7xl md:py-8 sm:px-6">
        <div className="flex flex-col justify-start mx-8 mb-4 md:mb-8 md:mx-0">
          <PortableText value={block} components={homeBlogSerializer} />
        </div>
        <div>
          <div className="grid grid-cols-1 gap-8 p-4 md:grid-cols-2 lg:mt-0 lg:grid-cols-2 md:p-10 ">
            {custom_blogs.map(({ author, excerpts, image, slug, title, _createdAt, _id }) => (
              <div
