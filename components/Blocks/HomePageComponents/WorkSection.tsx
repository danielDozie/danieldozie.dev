...
export const WorkSection = ({data}:DATA) => {
    const {title, left_image, content}: WORKSECTION = data;
    return (
        <div className="relative max-w-screen-xl p-4 px-4 mx-auto bg-white dark:bg-gray-800 sm:px-6 lg:px-8 py-26 lg:mt-20">
            ...
            <div className="flex justify-center p-4 mt-10 md:p-8">
                <h1 className="text-3xl font-semibold text-center text-gray-600 uppercase font-archivo dark:text-white">{title}</h1>
            </div>
                    ...
                    <div className="relative mt-10 lg:-mx-4 relative-20 lg:mt-0 lg:col-start-1">
                        <div className="relative space-y-4">
                            <div className="flex items-end justify-center space-x-4">
                                {left_image && (
                                    <Image className="w-full rounded-lg shadow-lg md:w-56" width={500} height={500} src={urlFor(left_image).url()} alt="left-image" />
                                )
                                }
                            </div>
                        </div>
