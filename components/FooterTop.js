                        ...
                        {Icons.map(({ name, title, description, index }) => (
                            <div className={`flex flex-col md:flex-row md:justify-between py-4 px-4 rounded-lg shadow-2xl  dark:border-gray-600 bottom-2`} key={index}>
                                ...
                                <div className="w-full flex flex-col ">
                                    <h1 className="font-mitr font-semibold text-sm md:text-lg bg-clip-text text-transparent bg-gradient-to-r from-green-400 to-blue-500">{title}</h1>
                                    <p className="font-mitr font-light text-sm mb-5 text-gray-700 dark:text-gray-200"> {description?.substr(0, 110)} <Link href={'/'}>...Read more</Link></p>
                                    <hr />
                                    <div className="flex text-xs font-thin mt-2 justify-between">
                                        <p className="text-gray-700 dark:text-gray-300">Published: <span className="text-pink-500">Sept. 2021</span></p>
                                        <p className="text-gray-700 dark:text-gray-300">Author: <span className="text-pink-500">Badt0men</span></p>
                                    </div>
